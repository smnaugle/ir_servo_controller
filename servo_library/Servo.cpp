...
int Servo::attach(uint pin) {
    return attach(pin, DEFAULT_MIN_PULSE_WIDTH, DEFAULT_MAX_PULSE_WIDTH);
}

int Servo::attach(uint pin, int minUs, int maxUs) {
    return attach(pin, minUs, maxUs, _valueUs);
}

int Servo::attach(uint pin, int minUs, int maxUs, int value) {
    ...
    if (!_attached) {
        bool success = pio_claim_free_sm_and_add_program_for_gpio_range(&servo_program, &_pio, &_smIdx, &_pgmOffset, _pin, 1, true);
        _pin = pin;
        hard_assert(success);
        _attached = true;
        servo_program_init(_pio, _smIdx, _pgmOffset, pin);
        pio_sm_set_enabled(_pio, _smIdx, false);
        pio_sm_put_blocking(_pio, _smIdx, usToPIOCycles(REFRESH_INTERVAL) / 3);
...
void Servo::detach() {
    if (_attached) {
        ...
        sleep_us(5);  // Avoid race condition
        do {
...
void Servo::writeMicroseconds(int value) {
    ...
    if (_attached) {
        ...
        pio_sm_put_blocking(_pio, _smIdx, usToPIOCycles(value) / 3);
    }
}
...
//     while(true){
//         servo.write(0);
//         printf("Servo is %b", servo.attached());
//         sleep_ms(1000);
//         servo.write(180);
//         sleep_ms(1000);
//         servo.write(90);
