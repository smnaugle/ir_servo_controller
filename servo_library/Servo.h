...
#define DEFAULT_NEUTRAL_PULSE_WIDTH  1250 // default duty cycle when servo is attached
...
public:
    ...
    // returns channel number or 0 if failure.
    int attach(uint pin, int min, int max);
    // attach the given pin to the next free channel, sets pinMode, min, and max values for write(),
    // and sets the initial value, the same as write().
    // returns channel number or 0 if failure.
    int attach(uint pin, int min, int max, int value);
    void detach();
...
private:
    ...
    uint  _pin;
    PIO      _pio; // PIO0/1
    uint      _smIdx;  // The state machine inside
    uint      _pgmOffset; // Where in the SM the code lives
    int _minUs;
    int _maxUs;
    int _valueUs;
